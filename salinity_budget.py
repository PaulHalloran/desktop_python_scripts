...
for model in models:
	...
	sw_density = 1.027 # (but should really change to use local ML averaged density)
...
plt.close('all')
plt.plot(ts1.data,'b')
plt.plot(-1.0 *(ts2.data - np.mean(ts2.data)),'r')
...
qplt.scatter(ts1,ts2[0:-1] - ts2[0:-1].collapsed('time',iris.analysis.MEAN))
plt.show()
