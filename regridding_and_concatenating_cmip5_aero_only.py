...
input_directory = '/data/temp/ph290/aero/'
...
output_directory = '/media/usb_external1/cmip5/aero2/'
...
variables = np.array(['tas','rsds','pr'])
		...
		for ens in ensls:
			...
			for i,var in enumerate(variables):
				...
				if np.size(tmp) == 0:
					...
					files = glob.glob(dir1+'/'+var+'*'+time_period+'*_'+model+'_*'+experiment+'_*'+ens+'*.nc')
					...
					if not sizing == 0:
						...
						try:
							...
							iris.fileformats.netcdf.save(cube, dir2+model+'_'+variables[i]+'_'+experiment+'_'+ens+'_regridded.nc','NETCDF3_CLASSIC')
							subprocess.call('rm '+temporary_file_space+temp_file4, shell=True)
						except:
							print model+' failed'
