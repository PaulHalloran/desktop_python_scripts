...
output_directory = '/media/usb_external1/cmip5/last1000/'
...
variables = np.array(['vo','pr','sos','tas','os','thetao'])
	...
	for experiment in experiments:
		...
		for i,var in enumerate(variables):
			...
			if np.size(tmp) == 0:
				...
				files = glob.glob(dir1+'/'+var+'*'+time_period+'*_'+model+'_*'+experiment+'_*r*.nc')
				...
				if not sizing == 0:
					...
					subprocess.call(['cdo -P 8 remapbil,r360x180 -selname,'+var+' '+temporary_file_space+temp_file2+' '+temporary_file_space+temp_file3], shell=True)
					...
					#regrid data onto common vertical levels
						subprocess.call(['cdo -P 8 intlevel,6000,5500,5000,4500,4000,3500,3000,2500,2000,1500,1000,900,800,700,600,500,400,300,200,100,90,80,70,60,50,40,30,20,10,5,0 '+temporary_file_space+temp_file3+' '+temporary_file_space+temp_file4], shell=True)
						subprocess.call('rm '+temporary_file_space+temp_file3, shell=True)
					else:
						subprocess.call('mv '+temporary_file_space+temp_file3+' '+temporary_file_space+temp_file4, shell=True)
					#print 'time-meaning data (to annual means if required)'		
					...
					#except:
					#	print model+' failed'
					subprocess.call('mv '+temporary_file_space+temp_file4+' '+dir2+model+'_'+variables[i]+'_'+experiment+'_regridded.nc', shell=True)
					subprocess.call('rm '+temporary_file_space+temp_file4, shell=True)
