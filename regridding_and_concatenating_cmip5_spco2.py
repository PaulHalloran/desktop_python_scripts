...
output_directory = '/data/temp/ph290/andy_w_analysis/processed_annual/'
...
for model in models:
	var_tmp = []
	files = glob.glob(input_directory+'/*'+time_period+'*_'+model+'_*r1i1p1*.nc')
	for file in files:
		var_tmp.append(file.split('_')[2].split('/')[1])
	var_tmp = np.unique(var_tmp)
	if ('spco2' in var_tmp) & ('tos' in var_tmp) & ('sos' in var_tmp) & ('intpp' in var_tmp):
		if ('mlotst' in var_tmp):
		...
		if np.size(tmp) == 0:
			...
			if not sizing == 0:
				...
				if test2 == 0:
					...
					#averaging to annual (if required)
