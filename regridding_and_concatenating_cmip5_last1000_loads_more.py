...
variables = np.array(['uo','vo','thetao','so','evs','pr','mlotst','tauu','tauv','wfo','sic'])
...
time_periods = np.array(['Amon','Omon'])
		...
		for ensemble in ensembles:
			...
			for experiment in experiments:
				...
				for i,var in enumerate(variables):
					...
					tmp = glob.glob(dir2+model+'_'+var+'_'+experiment+'*'+ensemble+'_regridded'+'*'+'time_period.nc')
					...
					if np.size(tmp) == 0:
						...
						files = glob.glob(dir1+'/'+var+'_*'+time_period+'*_'+model+'_*'+experiment+'*'+ensemble+'*.nc')
						...
						if not sizing == 0:
							...
							if (var == 'uo') or (var == 'vo'):
								print 'extract surface level'
								subprocess.call('cdo -P 6 sellevidx,1 '+temporary_file_space+temp_file2+' '+temporary_file_space+temp_file3, shell=True)
								...
								subprocess.call(['cdo -P 6 remapbil,r360x180 '+temporary_file_space+temp_file3+' '+dir2+model+'_'+var+'_'+experiment+'_'+ensemble+'_regridded_not_vertically_'+time_period+'.nc'], shell=True)
								...
								print 'regridding files horizontally'
								#then regrid data onto a 360x180 grid - you could change these values if you wanted to work with different resolurtoin data (lower resolution would make smaller files that would be quicker to work with)
								subprocess.call(['cdo -P 6 remapbil,r360x180 '+temporary_file_space+temp_file2+' '+dir2+model+'_'+var+'_'+experiment+'_'+ensemble+'_regridded_not_vertically_'+time_period+'.nc'], shell=True)
								subprocess.call('rm '+temporary_file_space+temp_file2, shell=True)
						else:
