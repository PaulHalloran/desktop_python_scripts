...
for model in models:
	...
	try:
		...
		composite_data[model]['composite_high_high'] = {}
		composite_data[model]['composite_low_low'] = {}
		composite_data[model]['composite_low_high'] = {}
		composite_data[model]['composite_high_low'] = {}
		for offset in offsets:
			...
			#output
			composite_data[model]['composite_high_high'][str(offset)] = cube2[loc_max2].collapsed('time',iris.analysis.MEAN)
			composite_data[model]['composite_low_low'][str(offset)] = cube2[loc_min2].collapsed('time',iris.analysis.MEAN)
			composite_data[model]['composite_low_high'][str(offset)] = cube2[loc_max2b].collapsed('time',iris.analysis.MEAN)
			composite_data[model]['composite_high_low'][str(offset)] = cube2[loc_min2b].collapsed('time',iris.analysis.MEAN)
...
for j,offset in enumerate(offsets):
	...
	for model in models_tmp:
		if np.size(np.shape(composite_data[model]['composite_high_high'][str(offset)])) == 3:
			composite_data[model]['composite_high_high'][str(offset)] = composite_data[model]['composite_high_high'][str(offset)][0]
			composite_data[model]['composite_low_low'][str(offset)] = composite_data[model]['composite_low_low'][str(offset)][0]
			composite_data[model]['composite_low_high'][str(offset)] = composite_data[model]['composite_low_high'][str(offset)][0]
			composite_data[model]['composite_high_low'][str(offset)] = composite_data[model]['composite_high_low'][str(offset)][0]
				

	for model in models_tmp:
		 if not(corr_variable == 'msftbarot'):
			#print model
			c1 = composite_data[model]['composite_low_low'][str(offset)]
			c2 = composite_data[model]['composite_high_high'][str(offset)]
			c3 = composite_data[model]['composite_low_high'][str(offset)]
			c4 = composite_data[model]['composite_high_low'][str(offset)]
			composite_data[model]['composite_low_low'][str(offset)].data = np.ma.masked_where(c1.data < -10000,c1.data)
			composite_data[model]['composite_high_high'][str(offset)].data = np.ma.masked_where(c2.data < -10000,c2.data)
			composite_data[model]['composite_low_low'][str(offset)].data = np.ma.masked_where(c1.data > 10000,c1.data)
			composite_data[model]['composite_high_high'][str(offset)].data = np.ma.masked_where(c2.data > 10000,c2.data)
			composite_data[model]['composite_low_high'][str(offset)].data = np.ma.masked_where(c3.data < -10000,c3.data)
			composite_data[model]['composite_high_low'][str(offset)].data = np.ma.masked_where(c4.data < -10000,c4.data)
			composite_data[model]['composite_low_high'][str(offset)].data = np.ma.masked_where(c3.data > 10000,c3.data)
			composite_data[model]['composite_high_low'][str(offset)].data = np.ma.masked_where(c4.data > 10000,c4.data)



	composite_mean_low_low = composite_data[models_tmp[0]]['composite_high_high'][str(offset)].copy()
	composite_mean_high_high = composite_data[models_tmp[0]]['composite_high_high'][str(offset)].copy()
	composite_mean_low_high = composite_data[models_tmp[0]]['composite_high_high'][str(offset)].copy()
	composite_mean_high_low = composite_data[models_tmp[0]]['composite_high_high'][str(offset)].copy()
	...
	for model in models_tmp:
			i += 1
			#print model
			composite_mean_data_low_low += composite_data[model]['composite_low_low'][str(offset)].data
			composite_mean_data_high_high += composite_data[model]['composite_high_high'][str(offset)].data
			composite_mean_data_low_high += composite_data[model]['composite_low_high'][str(offset)].data
			composite_mean_data_high_low += composite_data[model]['composite_high_low'][str(offset)].data
	...
	plt.title('high volcanic high solar '+str(offset))
	...
	bar = plt.colorbar(my_plot, orientation='horizontal', extend='both')
	bar.set_label(corr_variable+' ('+format(tmp_high_high.units)+')')
	plt.title('low volcanic low solar '+str(offset))
	...
	bar = plt.colorbar(my_plot, orientation='horizontal', extend='both')
	bar.set_label(corr_variable+' ('+format(tmp_high_high.units)+')')
	plt.title('high volcanic low solar '+str(offset))

	ax1 = plt.subplot(224,projection=ccrs.NorthPolarStereo(central_longitude=0.0))
	ax1.set_extent([-180, 180, 30, 90], crs=ccrs.PlateCarree())
	my_plot = iplt.contourf(tmp_low_high,np.linspace(min_use,max_use),cmap='bwr')
	#ax1.add_feature(cfeature.LAND,facecolor='#f6f6f6')
	plt.gca().coastlines()
	bar = plt.colorbar(my_plot, orientation='horizontal', extend='both')
	bar.set_label(corr_variable+' ('+format(tmp_high_high.units)+')')
	plt.title('low volcanic high solar '+str(offset))
	plt.savefig('/home/ph290/Documents/figures/fast_volc_and_low_variability_solar_composites_'+corr_variable+'_'+names[j]+'.png')
	#plt.show()
