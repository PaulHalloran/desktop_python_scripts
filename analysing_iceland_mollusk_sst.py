...
for model in models:
	...
	tmp2 = cube.extract(region2)
        try:   
                tmp2.coord('latitude').guess_bounds()
        except:
                print 'has bounds'
        try:   
                tmp2.coord('longitude').guess_bounds()
        except:
                print 'has bounds'
        grid_areas = iris.analysis.cartography.area_weights(tmp2)
	cubes_n.append(tmp2)
	ts_n.append(tmp2.collapsed(['latitude','longitude'],iris.analysis.MEAN,weights=grid_areas))
...
for model in modelsb:
	...
	#plt.show()
	ts_amob.append(tmp1.collapsed(['latitude','longitude'],iris.analysis.MEAN,weights=grid_areas))
	try:
                tmp1.coord('latitude').guess_bounds()
        except:
                print 'has bounds'
	...
	ts_nb.append(tmp2.collapsed(['latitude','longitude'],iris.analysis.MEAN,weights=grid_areas))
        try:
                tmp1.coord('latitude').guess_bounds()
        except:
                print 'has bounds'
	...
	tmp3 = cube.extract(region3)
        try:
                tmp3.coord('latitude').guess_bounds()
        except:
                print 'has bounds'
        try:   
                tmp3.coord('longitude').guess_bounds()
        except:
                print 'has bounds'
        grid_areas = iris.analysis.cartography.area_weights(tmp3)
	cubes_southb.append(tmp3)
	ts_southb.append(tmp3.collapsed(['latitude','longitude'],iris.analysis.MEAN,weights=grid_areas))
...
np.savetxt('/home/ph290/Documents/figures/sst_cmip5.txt', np.c_[year[0]+np.arange(1000),multimodel_mean])
...
plt.savefig('/home/ph290/Documents/figures/multi_model_mean_preindustrial_AMO_renolds_mann_proposal.png')
plt.show(block = False)
...
for model in modelsb:
	if model in ['CCSM4','MIROC-ESM', 'MPI-ESM-P']:
		...
		#qplt.contourf(tmp1[0])
		#plt.show()
		ts_amobs.append(tmp1.collapsed(['latitude','longitude'],iris.analysis.MEAN,weights=grid_areas))
		tmp2 = cube.extract(region2)
	        try:
	                tmp2.coord('latitude').guess_bounds()
	        except:
		...
		ts_nbs.append(tmp2.collapsed(['latitude','longitude'],iris.analysis.MEAN,weights=grid_areas))
		tmp3 = cube.extract(region3)
                try:   
                        tmp3.coord('latitude').guess_bounds()
                except:
		...
		ts_southbs.append(tmp3.collapsed(['latitude','longitude'],iris.analysis.MEAN,weights=grid_areas))
...
np.savetxt('/home/ph290/Documents/figures/sss_cmip5.txt', np.c_[year[0]+np.arange(1000),multimodel_means])
...
ax1 = plt.subplot(5,1,5)
y = running_mean_post.running_mean_post(solar_data,20)
ax1.plot(solar_year[20::],(y[20::]-np.mean(y[20::]))*-1.0,'pink')
#ax1.plot(data_final[:,0],running_mean_post.running_mean_post(data_final[:,1],12.0*40.0)*10,'k')
ax1.set_xlim([900,1900])
#ax1.set_ylim([-2,2])
ax1.set_xlabel('year')
...
loc = np.where((data_final_t[:,0] >= np.min(year2)) & (data_final_t[:,0] <= np.max(year2)))[0]
...
locb = np.where((data_final_t[:,0] >= np.min(x_reynolds)) & (data_final_t[:,0] <= np.max(x_reynolds)))[0]
volcb = running_mean_post.running_mean_post(data_final_t[locb,1],12.0*150.0)
...
plt.title('LIA -ve correlation swith temperature')
ax2.set_yscale('log')
...
ax1 = plt.subplot(1,1,1)
#ax1.scatter(y2,x2)
ax1.plot(year2,y2,'b',linewidth=3,alpha = 0.8)
ax1.plot(year2,multimodel_mean,'b',alpha = 0.3)
#ax1.plot(x_reynolds,y2b*-0.5,'b',linewidth=3)
ax2 = ax1.twinx()
ax2.plot(year2,x2,'r',linewidth=3,alpha = 0.8)
ax2.plot(year2,f_unsmoothed(np.linspace(0,np.size(volc_unsmoothed),np.size(multimodel_mean))),'r',alpha = 0.3)
...
ax2.set_ylabel('30-90N volcanic AOD (log scale)')
ax1.set_ylabel('-1*model Iceland SST')
plt.title('MCA +ve correlation swith temperature')
ax2.set_yscale('log')

plt.savefig('/home/ph290/Documents/figures/volcanics_explain_n_iceland_tb.png')
#plt.show(block = True)
...
#temperature volcanics


year2 = year[0]+np.arange(1000)
#year2 = 1300+np.arange(550)
#y = running_mean.running_mean(multimodel_mean-np.mean(multimodel_mean),10)
y_t = (multimodel_mean-np.mean(multimodel_mean))
#y = multimodel_mean[450::]-np.mean(multimodel_mean[450::])
y_t = y_t[np.logical_not(np.isnan(y_t))]
yr = year2[np.logical_not(np.isnan(y_t))]



volcanic_smoothing = 65 #yrs
model_smoothing = 2 #yrs

loc = np.where((data_final_s[:,0] >= np.min(year2)) & (data_final_s[:,0] <= np.max(year2)))[0]
volc = running_mean_post.running_mean_post(data_final_s[loc,1],12.0*volcanic_smoothing)
volc[np.where(volc == 0.0)] = 1.0e-4
volc_unsmoothed = data_final_s[loc,1]
volc_unsmoothed[np.where(volc_unsmoothed == 0.0)] = 1.0e-4
...
locb = np.where((data_final_s[:,0] >= np.min(x_reynolds)) & (data_final_s[:,0] <= np.max(x_reynolds)))[0]
volcb = running_mean_post.running_mean_post(data_final_s[locb,1],12.0*150.0)
...
ax1 = plt.subplot(1,1,1)
#ax1.scatter(y2,x2)
ax1.plot(yr,running_mean_post.running_mean_post(signal.detrend(y_t),model_smoothing),'b',label = 'CMIP5 multimodel\nmean SST (*-1.0)',linewidth=3,alpha = 0.8)
...
ax2 = ax1.twinx()
ax2.semilogy(year2,x2,'r',linewidth=3,alpha = 0.8)
ax2.semilogy(year2,f_unsmoothed(np.linspace(0,np.size(volc_unsmoothed),np.size(multimodel_mean))),'r',alpha = 0.3,linewidth= 3)
#ax2.plot(x_reynolds,x2b,'r',linewidth=3)
...
ax1.set_ylabel('model Iceland SST')

#ax2.set_yscale('log')
plt.savefig('/home/ph290/Documents/figures/volcanics_explain_n_iceland_t_b.png')
#plt.show(block = True)
...
ax1 = plt.subplot(1,1,1)
#ax1.scatter(y2,x2)
ax1.plot(yr,running_mean_post.running_mean_post(signal.detrend(y_t),model_smoothing)*-1.0,'b',label = 'CMIP5 multimodel\nmean SST (*-1.0)',linewidth=3,alpha = 0.8)
...
ax1.set_xlabel('year')
ax2.set_ylabel('S. hemisphere volcanic AOD (log scale)')
ax1.set_ylabel('model Iceland SST (inverted)')

#ax2.set_yscale('log')
plt.savefig('/home/ph290/Documents/figures/volcanics_explain_n_iceland_t_b_inverted.png')
#plt.show(block = True)
...
#year2 = 1300+np.arange(550)
#y = running_mean.running_mean(multimodel_mean-np.mean(multimodel_mean),10)
y_sal = (multimodel_means-np.mean(multimodel_means))*-1.0
#y = multimodel_mean[450::]-np.mean(multimodel_mean[450::])
y_sal = y_sal[np.logical_not(np.isnan(y_sal))]
yr = year2[np.logical_not(np.isnan(y_sal))]
...
ax1 = plt.subplot(1,1,1)
#ax1.scatter(y2,x2)
ax1.plot(yr,running_mean_post.running_mean_post(signal.detrend(y_sal),model_smoothing),'g',label = 'CMIP5 multimodel\nmean salinity (*-1.0)',linewidth=3,alpha = 0.8)
...
#year2 = 1300+np.arange(550)
#y = running_mean.running_mean(multimodel_mean-np.mean(multimodel_mean),10)
y = (multimodel_mean-np.mean(multimodel_mean))*-1.0
#y = multimodel_mean[450::]-np.mean(multimodel_mean[450::])
y = y[np.logical_not(np.isnan(y))]
yr = year2[np.logical_not(np.isnan(y))]
...
#plt.close('all')
plt.figure(3,figsize=(14, 5),dpi=60)
ax1 = plt.subplot(1,1,1)
#ax1.scatter(y2,x2)
ax1.plot(yr,running_mean.running_mean(signal.detrend(y),1),'b',linewidth = 2, alpha=0.5,label = 'CMIP5 multimodel\nmean SST (*-1.0)')
#ax1.plot(x_reynolds,y2b,'b',linewidth=3)
ax2 = ax1.twinx()
y = running_mean_post.running_mean_post(solar_data,10)
ax2.plot(solar_year[20::],(y[20::]-np.mean(y[20::]))*-1.0,'red',alpha=0.8,linewidth=3)
#ax2.plot(solar_year[20::],(y[20::]-np.mean(y[20::])),'r',linewidth=3,alpha = 0.8)
ax1.set_xlim([900,1900])
...
#ax2.set_ylim([1.0e-4,1.0e0])
ax1.set_xlabel('year')
ax2.set_ylabel('solar (red)')
ax1.set_ylabel('model Iceland SST')

#ax2.set_yscale('log')
plt.savefig('/home/ph290/Documents/figures/solar_explain_n_iceland_t.png')
#plt.show(block = False)
...
#year2 = 1300+np.arange(550)
#y = running_mean.running_mean(multimodel_mean-np.mean(multimodel_mean),10)
y = (multimodel_means-np.mean(multimodel_means))
#y = multimodel_mean[450::]-np.mean(multimodel_mean[450::])
y = y[np.logical_not(np.isnan(y))]
yr = year2[np.logical_not(np.isnan(y))]
...
ax1 = plt.subplot(1,1,1)
#ax1.scatter(y2,x2)
ax1.plot(yr,running_mean.running_mean(signal.detrend(y),2),'g',linewidth = 2, alpha=0.5,label = 'CMIP5 multimodel\nmean salinity (*-1.0)')
#ax1.plot(x_reynolds,y2b,'b',linewidth=3)
ax2 = ax1.twinx()
y = running_mean_post.running_mean_post(solar_data,10)
ax2.plot(solar_year[20::],(y[20::]-np.mean(y[20::])),'red',alpha=0.8,linewidth=3)
#ax2.plot(solar_year[20::],(y[20::]-np.mean(y[20::])),'r',linewidth=3,alpha = 0.8)
ax1.set_xlim([900,1900])
...
#ax2.set_yscale('log')
plt.savefig('/home/ph290/Documents/figures/solar_explain_n_iceland_s.png')
#plt.show(block = False)
...
year_r = reynolds_data[:,0]
#year2 = 1300+np.arange(550)
#y = running_mean.running_mean(multimodel_mean-np.mean(multimodel_mean),10)
y = (reynolds_data[:,1]-np.mean(reynolds_data[:,1]))
#y = multimodel_mean[450::]-np.mean(multimodel_mean[450::])
y = y[np.logical_not(np.isnan(y))]
yr_r = year_r[np.logical_not(np.isnan(y))]
...
ax1 = plt.subplot(1,1,1)
#ax1.scatter(y2,x2)
ax1.plot(yr_r,running_mean.running_mean(signal.detrend(y),1),'k',linewidth = 2, alpha=0.5,label = 'Reynolds d18O')
#ax1.plot(x_reynolds,y2b,'b',linewidth=3)
ax2 = ax1.twinx()
y = running_mean_post.running_mean_post(solar_data,10)
ax2.plot(solar_year[20::],(y[20::]-np.mean(y[20::]))*-1.0,'red',alpha=0.8,linewidth=3)
...
ax1.set_xlabel('year')
ax2.set_ylabel('solar (red)')
ax1.set_ylabel('N. Iceland d18O')

#ax2.set_yscale('log')
plt.savefig('/home/ph290/Documents/figures/solar_explain_n_iceland_d18o.png')
#plt.show(block = False)
...
ax2.set_ylabel('surface salinity')
plt.legend()
plt.title('model T-S relationships')
#plt.savefig('/home/ph290/Documents/figures/model_t_s_relationships.png')
plt.show(block = False)
