...
for j,offset in enumerate(offsets):
	...
	for model in models:
		...
		min_loc = np.where(ts_variable <= np.mean(ts_variable) - np.mean(ts_variable))
		max_loc = np.where(ts_variable >= np.mean(ts_variable) + np.mean(ts_variable))
	...
	plt.savefig('/home/ph290/Documents/figures/tas_composites_'+names[j]+'.png')
	#plt.show()
