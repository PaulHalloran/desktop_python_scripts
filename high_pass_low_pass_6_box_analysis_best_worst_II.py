...
for i,loc in enumerate(even_indices):
	ax1 = fig.add_subplot(nrow,ncol,loc)
	lns=plt.plot(qump_year,qump_data[0,:,best[i]],c='k',linewidth=2,label='ESPPE flux',alpha = alpha_val)
...
for i,loc in enumerate(odd_indices):
	ax1 = fig.add_subplot(nrow,ncol,loc)
	lns=plt.plot(qump_year,qump_data[0,:,worst[i]],c='k',linewidth=2,label='ESPPE flux',alpha = alpha_val)
	for k in np.arange(3):
...
labs = [l.get_label() for l in lns]
plt.legend(lns, labs, loc=2,prop={'size':9.6},bbox_to_anchor=(-1.3, 5.05)).draw_frame(False)
...
plt.subplots_adjust(left=0.1, bottom=0.1, right=0.9, top=0.9, wspace=0.25, hspace=0.35)
