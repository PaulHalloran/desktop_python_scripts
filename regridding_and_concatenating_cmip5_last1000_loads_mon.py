...
output_directory = '/data/NAS-ph290/ph290/cmip5/last1000/monthly/'
...
variables = np.array(['evspsbl','sos','tas','rsdt','rsus','rsds','sfcWind','sic','vas','uas','hurs','hur','so','ua','va'])
#later add va and ua
...
time_periods = np.array(['Amon','Omon','OImon'])
				...
				for i,var in enumerate(variables):
					...
					if np.size(tmp) == 0:
						...
						if not sizing == 0:
							...
							subprocess.call(['cdo -P 6 remapbil,r360x180 -selname,'+var+' '+temporary_file_space+temp_file1+' '+dir2+model+'_'+variables[i]+'_'+experiment+'_'+ensemble+'_regridded.nc'], shell=True)
