...
tmp_shape=box_co2_flux[:,:,0,0].shape

# fig = plt.figure(figsize=(5*3.13,3*3.13))
# for i,k in enumerate(np.arange(15,21)):  
#     ax1 = fig.add_subplot(2,4,i+1)
#     for j in np.arange(np.size(filenamesb)):
...
plot_names2=['param. set 1','param. set 2','param. set 3','param. set 4','param. set 5']

lns=[]
ln_tmp=[]

titlesa = ['highest R$^2$','2$^{nd}$ highest R$^2$','3$^{rd}$ highest R$^2$','4$^{th}$ highest R$^2$']
titlesb = ['lowest R$^2$','2$^{nd}$ lowest R$^2$','3$^{rd}$ lowest R$^2$','4$^{th}$ lowest R$^2$']
...
ax0 = fig.add_subplot(nrow+1,1,1)
n, bins, patches = plt.hist(r2_sorted, 7, normed=1, histtype='stepfilled')
plt.setp(patches, 'facecolor', 'gray', 'alpha', 0.75)
plt.yticks([0,2,4])
plt.xlabel('R$^2$',size = 'x-large')
plt.ylabel('frequency',size = 'x-large')


for i,loc in enumerate(even_indices):
	ax1 = fig.add_subplot(nrow+1,ncol,loc+2)
	lns=plt.plot(qump_year,qump_data[0,:,worst[i]],c='k',linewidth=2,label='ESPPE flux',alpha = 1)
	for k in np.arange(3):
		ln_tmp=plt.plot(qump_year,(box_co2_flux[:,worst[i],0,k]/1.12e13)*1.0e15/12.0,label=plot_names2[k],alpha = alpha_val,linewidth=lw)
		plt.title(titlesb[i]+' ('+str(r2_sorted[i])[0:4]+')')
		lns += ln_tmp
	plt.xlim(1860,2100)
...
for i,loc in enumerate(odd_indices):
	ax1 = fig.add_subplot(nrow+1,ncol,loc+2)
	lns=plt.plot(qump_year,qump_data[0,:,best[i]],c='k',linewidth=2,label='ESPPE flux',alpha = 1)
	for k in np.arange(3):
		ln_tmp=plt.plot(qump_year,(box_co2_flux[:,best[i],0,k]/1.12e13)*1.0e15/12.0,label=plot_names2[k],alpha = alpha_val,linewidth=lw)
		plt.title(titlesa[i]+' ('+str(r2_sorted[-1*(i+1)])[0:4]+')')
		lns += ln_tmp
	plt.xlim(1860,2100)
	plt.locator_params(axis = 'x', nbins = 4)
	plt.locator_params(axis = 'y', nbins = 4)
...
labs = [l.get_label() for l in lns]
plt.legend(lns, labs, loc=2,prop={'size':9.6},bbox_to_anchor=(-0.05, 4.225)).draw_frame(False)

fig.text(0.035, 0.5, 'CO$_{2}$ flux (umol m$^{-2}$ yr$^{-1}$)', ha='center', va='center', rotation='vertical',size = 'x-large')
fig.text(0.5, 0.05, 'Year', ha='center', va='center',size = 'x-large')

plt.subplots_adjust(left=0.1, bottom=0.1, right=0.9, top=0.9, wspace=0.25, hspace=0.60)



#plt.tight_layout()
#plt.show(block = False)
plt.savefig('/home/ph290/Documents/figures/n_atl/figure3_jul14.png')
