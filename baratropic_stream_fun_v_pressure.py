...
cuttoff=100.0
################
Wn_mid_high=timestep_between_values/cuttoff
b1, a1 = scipy.signal.butter(N, Wn_mid_high, btype='high')
################
cuttoff=10.0
################
Wn_mid_high=timestep_between_values/cuttoff
b2, a2 = scipy.signal.butter(N, Wn_mid_high, btype='low')


directory = '/media/usb_external1/cmip5/piControl/'
...
for model in models:
	...
	data[model]['msftbarot'] = scipy.signal.filtfilt(b1, a1, spg_strength)
	data[model]['msftbarot'] = scipy.signal.filtfilt(b2, a2, data[model]['msftbarot'])

maps = {}

for model in models:
	print model
        data[model]['msftbarot']  = np.ma.masked_invalid(data[model]['msftbarot'])
...
plt.title('Strong Subpolar Gyre')
...
plt.gca().coastlines()
bar = plt.colorbar(my_plot,ticks=[zmin2, 0, zmax2], orientation='horizontal', extend='both')
bar.set_label(all_data_high_mean_cube.long_name+' ('+format(all_data_high_mean_cube.units)+')')
plt.title('High minus Low Subpolar Gyre')

#plt.show(block = True)
plt.savefig('/home/ph290/Documents/figures/spg_mslp_1_smoothed.png')
...
#plt.show(block = False)
plt.savefig('/home/ph290/Documents/figures/spg_mslp_2_smoothed.png')
