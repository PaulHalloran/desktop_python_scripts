...
variables = np.array(['msftbarot','zos','wmo','uo'])
				...
				if np.size(tmp) == 0:
					...
					if not sizing == 0:
						if sizing > 1:
							...
							subprocess.call(['cdo -P 10 mergetime '+files+' '+temporary_file_space+temp_file1], shell=True)
						if sizing == 1:
						...
						subprocess.call('cdo -P 10 yearmean '+temporary_file_space+temp_file1+' '+temporary_file_space+temp_file2, shell=True)
						...
						subprocess.call(['cdo -P 10 remapbil,r360x180 -selname,'+var+' '+temporary_file_space+temp_file2+' '+dir2+model+'_'+variables[i]+'_'+experiment+'_'+ensemble+'_regridded.nc'], shell=True)
