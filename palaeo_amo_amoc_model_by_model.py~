...
for i in range(6):
	...
	ax1.plot(data6[50:-50],'r')
	ax2 = ax1.twinx()
	data6 = density_data[model]['evspsbl']
	data6 = scipy.signal.filtfilt(b, a, data6)
	data6 = rm.running_mean(data6,smoothing_val)
	x = data6
	x=(x-np.nanmin(x))/(np.nanmax(x)-np.nanmin(x))
...
'''
#Basically salinity is controlled by evap!
#temperature and salinity co-vary here, but warm at same time as salty, and density controlled by salty. Warmth could be driving exaporation, what is causing the warming?
'''
...
'''
for corr_variable in ['uas','vas','tas','hurs','sos','pr','psl','tos','evspsbl','hfss','rsntds','sic','rlds','hfls']:
    #corr_variable = 'sos'
    models_tmp = []
    composite_data = {}
    ts_variable = 'sos'
