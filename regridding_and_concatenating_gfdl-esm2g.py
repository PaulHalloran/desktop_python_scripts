...
input_directory = '/data/NAS-ph290/shared/GFDL-ESM2G_SPCO2_RCP85/'
...
output_directory = '/data/NAS-ph290/shared/out/'
...
latitude= iris.coords.DimCoord(np.arange(-89.5, 90.5, 1), standard_name='latitude', units='degrees')
longitude = iris.coords.DimCoord(np.arange(-179.5, 180.5, 1), standard_name='longitude', units='degrees')
grid_cube = iris.cube.Cube(np.zeros((180, 360), np.float32),standard_name='surface_partial_pressure_of_carbon_dioxide_in_sea_water', long_name='surface partial pressure of CO2 in sea water', var_name='spco2', units='Pa',dim_coords_and_dims=[(latitude, 0), (longitude, 1)])
				...
				for i,var in enumerate(variables):
					...
					if np.size(tmp) == 0:
						...
						if not sizing == 0:
							...
							cube2 = iris.analysis.interpolate.regrid(cube, grid_cube, mode='bilinear')
							iris.fileformats.netcdf.save(cube2, dir2+model+'_'+variables[i]+'_'+experiment+'_regridded.nc','NETCDF3_CLASSIC')
