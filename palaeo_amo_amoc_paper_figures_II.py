...
amo_data = high_pass( amo_data,smoothing_window)
x = amo_data
...
tmp = r_data[:,1]
tmp = high_pass( tmp,smoothing_window)
tmp = rm.running_mean(tmp,smoothing_val)
...
era_cube = era_cube[:-1]
era_cube.data = high_pass( era_cube.data,axis = 0)
...
jfm_nao_data = high_pass( jfm_nao_data)
...
for i,model in enumerate(models):
	...
	try:
		...
		print 'no precipitation depth coordinate'
	pr_cube.data = high_pass( pr_cube.data,axis = 0)
	# years
...
voln_data2 = rnp.running_mean_post(voln_data2,1)
