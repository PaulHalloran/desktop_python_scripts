...
files2 = np.array(glob.glob(directory+'tas/piControl/*.nc'))
...
for k,model in enumerate(models_unique):
    files = np.array(glob.glob(directory+'tas/piControl/*'+model+'*.nc'))
    cubes = iris.load(files,'rainfall_flux',callback = my_callback)
    if len(cubes) == 0:
        cubes = iris.load(files,'air_temperature',callback = my_callback)
...
'''
High AMOC minus low AMOC tasecipitation:
'''
...
for k,model in enumerate(models_unique):
    ...
    cmip5_tas_amoc_diff.append(cube)


for k,model in enumerate(models_unique):
    tmp = cmip5_tas_amoc_diff[k]
    plt.figure()
    ax = qplt.contourf(tmp,50)
    #ax.coastlines()
    plt.title(model)
    plt.savefig('/home/ph290/Desktop/delete/'+model+'_tas_amoc.png')
    #plt.show()

'''
sort out plotting when I have more tas data downloiaded
'''
...
for k,model in enumerate(models_unique):
    plt.figure()
    tmp = cmip5_tas_amoc_diff[k]
    qplt.contourf(tmp,50)
    plt.gca().coastlines()
    plt.title(model+' AMOC composite')
    plt.savefig('/home/ph290/Desktop/delete/'+model+'_tas_amoc_global.png')
    #plt.show()
...
cmip5_tas_amoc_diff_regrid = np.copy(cmip5_tas_amoc_diff)

for k,model in enumerate(models_unique):
    cmip5_tas_amoc_diff_regrid[k] = iris.analysis.interpolate.regrid(cmip5_tas_amoc_diff[k],destination_cube1)

in_cube = iris.analysis.maths.add(cmip5_tas_amoc_diff_regrid[0],cmip5_tas_amoc_diff_regrid[1])
for k in np.arange(2,models_unique.size):
    in_cube = iris.analysis.maths.add(in_cube,cmip5_tas_amoc_diff_regrid[k])

cmip5_tas_amoc_diff_regrid_mean = iris.analysis.maths.divide(in_cube,models_unique.size)

cmip5_tas_amoc_diff_digital_plus = np.copy(cmip5_tas_amoc_diff)
for k,model in enumerate(models_unique):
    cmip5_tas_amoc_diff_digital_plus[k] = digital_plus(iris.analysis.interpolate.regrid(cmip5_tas_amoc_diff[k],destination_cube2)).copy()

in_cube = iris.analysis.maths.add(cmip5_tas_amoc_diff_digital_plus[0],cmip5_tas_amoc_diff_digital_plus[1])
for k in np.arange(2,models_unique.size):
    in_cube = iris.analysis.maths.add(in_cube,cmip5_tas_amoc_diff_digital_plus[k])

out_cube_plus = in_cube

cmip5_tas_amoc_diff_digital_minus = np.copy(cmip5_tas_amoc_diff)
for k,model in enumerate(models_unique):
    cmip5_tas_amoc_diff_digital_minus[k] = digital_minus(iris.analysis.interpolate.regrid(cmip5_tas_amoc_diff[k],destination_cube2)).copy()

in_cube = iris.analysis.maths.add(cmip5_tas_amoc_diff_digital_minus[0],cmip5_tas_amoc_diff_digital_minus[1])
for k in np.arange(2,models_unique.size):
    in_cube = iris.analysis.maths.add(in_cube,cmip5_tas_amoc_diff_digital_minus[k])

out_cube_minus = in_cube

cmip5_tas_amoc_diff_digital2 = out_cube_minus.copy()
cmip5_tas_amoc_diff_digital2.data = cmip5_tas_amoc_diff_digital2.data*0.0+np.nan
cmip5_tas_amoc_diff_digital2.data[np.where(out_cube_plus.data >= 3)] = 1.0
cmip5_tas_amoc_diff_digital2.data[np.where(out_cube_minus.data >= 3)] = 1.0

# plt.figure()
# qplt.contourf(cmip5_tas_amoc_diff_regrid_mean,50)
# points = iplt.points(cmip5_tas_amoc_diff_digital2, c =cmip5_tas_amoc_diff_digital2.data, s= 2.0)
# plt.gca().coastlines()
# plt.title('AMOC mean temperature change and agreement (3 out of 4)')
# plt.show()
...
tmp = cmip5_tas_amoc_diff_regrid_mean.intersection(longitude=(west, east))
tmp = tmp.intersection(latitude=(south, north))

tmp2 = cmip5_tas_amoc_diff_digital2.intersection(longitude=(west, east))
...
plt.gca().coastlines()
plt.title('AMOC mean temperature change and agreement (3 out of 4)')
plt.savefig('/home/ph290/Documents/figures/tas_amoc.png')
...
#     plt.title(model+' AMOC tasecipitation composite')
#     #plt.gca().coastlines()
#     #plt.gca().add_feature(cfeature.LAND)
#     plt.savefig('/home/ph290/Desktop/delete/'+model+'_tas_amoC_correct_question.png')
#     #plt.show()
...
# tas = iris.analysis.maths.np.subtract(cube_high_amo_mean_tas,cube_low_amo_mean_tas)
# qplt.contourf(tas,60)
# plt.gca().coastlines()
# plt.show()
