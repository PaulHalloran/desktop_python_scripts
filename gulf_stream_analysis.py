...
def model_names(directory):
	files = glob.glob(directory+'*piControl*.nc')
	models_tmp = []
	for file in files:
...
remove = np.where(models == 'HadGEM2-CC')
#remove inmcm4 because goes funny in the regridding...
models = list(models)
models.pop(remove[0])
models = np.array(models)
remove = np.where(models == 'CanESM2')
#remove inmcm4 because goes funny in the regridding...
models = list(models)
models.pop(remove[0])
models = np.array(models)
...
for model in models:
	...
	#changed now to look at NAC rather than gulf stream - perhaps revert back...
	...
	for i,year_tmp in enumerate(years):
		...
		if np.size(loc) == 1:
			if gulf_stream_lat[i] > mean_gs_lat+2.0:
...
fig = plt.figure()
qplt.contourf(mean_response,np.linspace(-1.0,1.0,51),cmap=cmap)
plt.gca().coastlines()
