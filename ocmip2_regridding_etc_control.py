...
all_files = glob.glob('/home/ph290/data0/ocmip/dods.ipsl.jussieu.fr/ocmip/phase2/*/Abiotic/ctrl/ctrlF/*.nc')
...
for model in models:
    files_1 = glob.glob(input_directory+'*'+model+'*/Abiotic/ctrl/ctrlF/*_2Da.nc')
        ...
        if np.size(tmp) == 0:
            ...
            if not sizing == 0:
                if sizing > 1:
                    ...
                    d14c_cube = iris.load_cube(temporary_file_space+temp_file1,'Delta C-14')
                    dpco2_cube = iris.load_cube(temporary_file_space+temp_file1,'Delta pCO2')
                    ...
                    for yr in range(d14c_cube.shape[0]):
                        ...
                        for i,lat_tmp in enumerate(theory_lats):
                            for j,lon_tmp in enumerate(theory_lons):
                                ...
                                #test = grid_mask.data[0][0][loc[0]]
                                #if np.size(test) > 0:
                                #    count+= 1
                                #    data_for_cube.mask[yr,i,j] = False
                    ...
                    iris.fileformats.netcdf.save(new_d14c_cube, output_directory+model+'_d14c_ctrlF.nc')
                    iris.fileformats.netcdf.save(new_dpco2_cube, output_directory+model+'_dpco2_ctrlF.nc')
